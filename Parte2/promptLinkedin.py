"""
ANÁLISIS DE SENTIMIENTOS EN REDES SOCIALES - TWITTER/X
Análisis exhaustivo de comentarios con 7 técnicas de NLP
Genera JSON con datos + interpretación LLM (Cohere)
"""
...
try:
    ...
    print("⚠️  cohere no instalado. Instalar con: pip install cohere")
...
if len(sys.argv) >= 3:
    ...
    CSV_INPUT_PATH = "datos_limpios/linkedin_limpio.csv"
    TEMA_ANALISIS = "nicolas maduro capturado"

#############################################
# CONFIGURACIÓN PRINCIPAL - COHERE
#############################################
...
def configurar_cohere():
    """Configura el cliente de Cohere"""
    try:
        if not COHERE_AVAILABLE:
            raise Exception("Cohere no está instalado")
        co = cohere.Client(COHERE_API_KEY)
        return co
    except Exception as e:
        raise Exception(f"Error configurando Cohere: {e}")
...
def interpretar_con_cohere(resumen_texto, tema, co_client):
    """Envía el resumen a Cohere para interpretación"""
    print("\n🤖 Enviando análisis a Cohere para interpretación...")
...
Basándote ÚNICAMENTE en estos datos estadísticos, proporciona una interpretación 
profesional, clara y detallada que incluya:
...
Tu interpretación debe ser objetiva, basada en evidencia, y expresada en un 
lenguaje profesional pero accesible. Evita especular más allá de lo que los 
datos muestran."""
    ...
    try:
        ...
        print("✅ Interpretación recibida de Cohere")
        return interpretacion, prompt
        
    except Exception as e:
        print(f"❌ Error al llamar a Cohere: {e}")
...
def main():
    ...
    print("\n🔧 Configurando Cohere...")
    try:
        co_client = configurar_cohere()
        print("✅ Cohere configurado")
    except Exception as e:
        print(f"❌ Error configurando Cohere: {e}")
    ...
    interpretacion, prompt_usado = interpretar_con_cohere(resumen_para_llm, TEMA_ANALISIS, co_client)
    
    datos_analisis['interpretacion_llm'] = {
        'timestamp': datetime.now().isoformat(),
        'modelo_usado': COHERE_MODEL,
    ...
    print(f"\n🤖 Interpretación de Cohere guardada en el JSON")
